...
export const usePayload = () => {
   ...
   * @param  {PayloadRequestParams} params? - Query parameters
   * @returns Promise<T>
   */
  const find = <T>(contentType: string, params?: PayloadRequestParams, fetchOptions?: FetchOptions): Promise<T> => {
   ...
   * @returns Promise<T>
   */
  const findOne = <T>(contentType: string, id?: string | number | PayloadRequestParams, params?: PayloadRequestParams, fetchOptions?: FetchOptions): Promise<T> => {
    if (typeof id === 'object') {
      params = id
      // @ts-ignore
      id = undefined
